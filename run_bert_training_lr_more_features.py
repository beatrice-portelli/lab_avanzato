...
parser.add_argument("--out_dir", default="combined_models_more_features", help="set root directory for all results (default ./combined_models_more_features)")
...
original_data_path = "/mnt/HDD/bportelli/lab_avanzato/beatrice_mod.pkl"
...
# FEATURE EXTRACTION FROM OTHER COLUMNS

from sklearn.preprocessing import MinMaxScaler, OneHotEncoder

eta = diagnosis_df_annotated["Età"].values.tolist()
eta = np.array(eta).reshape(-1,1)
eta_scaler = MinMaxScaler()
eta_scaler.fit(eta)
eta_features = eta_scaler.transform(eta)

sesso = diagnosis_df_annotated["Sesso"].values.tolist()
sesso = np.array(sesso).reshape(-1,1)
sesso_ohe = OneHotEncoder(handle_unknown='ignore')
sesso_ohe.fit(sesso)
sesso_features = sesso_ohe.transform(sesso).toarray()

all_extra_features = np.concatenate((eta_features, sesso_features), axis=1)

# (end) FEATURE EXTRACTION FROM OTHER COLUMNS
...
if args.do_train:
    ...
    for training_indexes, test_indexes in sk_fold.split(X=np.zeros(len(codes)), y=codes):
        ...
        for index in training_indexes:
            training_data_bert.append(features[index])
            training_data.append(all_extra_features[index])
            training_labels.append(all_label_codes[index])
        for index in test_indexes:
            test_data_bert.append(features[index])
            test_data.append(all_extra_features[index])
            test_labels.append(all_label_codes[index])

        # Convert to Tensors and build dataset
        all_unique_ids = torch.tensor([f.unique_id for f in training_data_bert], dtype=torch.long) 
        all_input_ids = torch.tensor([f.input_ids for f in training_data_bert], dtype=torch.long)
        all_input_mask = torch.tensor([f.input_mask for f in training_data_bert], dtype=torch.long)
        all_segment_ids = torch.tensor([f.segment_ids for f in training_data_bert], dtype=torch.long)
        # all_label_ids = torch.tensor([f.label_id for f in training_data_bert], dtype=torch.long)
        ...
        if os.path.exists(data_serialized) and not line_args.overwrite:
            ...
            data = {}
            data["Training-Data-BERT"] = training_data_bert
            data["Training-Data"] = training_data
            data["Training-Labels"] = training_labels
            data["Test-Data-BERT"] = test_data_bert
        ...
        if os.path.exists(model_output_dir+"/logistic_regression.bin") and not line_args.overwrite:
            ...
            training_data_bert_emb = np.asarray(all_embeddings)
            
            # additional features (e.g. sesso, eta, ...)
            training_data = np.asarray(training_data)
            
            # concatenation of all features ( additional features(+)bert_embeddings )
            ml_train_data = np.concatenate((training_data, training_data_bert_emb), axis=1)
...
if args.do_eval:
    ...
    for fold_counter in range(1, folds_number+1):
        ...
        with open(data_serialized, "rb") as input_file:
            training_data_bert = pickle.load(input_file)

        test_data_bert = training_data_bert["Test-Data-BERT"]
        test_data = training_data_bert["Test-Data"]
        test_labels = training_data_bert["Test-Labels"]
        
        # Actual prediction

        # Convert to Tensors and build dataset
        all_unique_ids = torch.tensor([f.unique_id for f in test_data_bert], dtype=torch.long) 
        all_input_ids = torch.tensor([f.input_ids for f in test_data_bert], dtype=torch.long)
        all_input_mask = torch.tensor([f.input_mask for f in test_data_bert], dtype=torch.long)
        all_segment_ids = torch.tensor([f.segment_ids for f in test_data_bert], dtype=torch.long)
        all_label_ids = torch.tensor([f.label_id for f in test_data_bert], dtype=torch.long)
        ...
        # BERT embeddings
        all_embeddings = all_embeddings.cpu()
        test_data_bert_emb = np.asarray(all_embeddings)
            
        # additional features (e.g. sesso, eta, ...)
        test_data = np.asarray(test_data)
            
        # concatenation of all features ( additional features(+)bert_embeddings )
        ml_test_data = np.concatenate((test_data, test_data_bert_emb), axis=1)
